document.addEventListener('DOMContentLoaded', function () {
        ...
        if (navigator.permissions) {
            navigator.permissions.query({ name: 'geolocation' }).then(function (permissionStatus) {
                if (permissionStatus.state === 'granted') {
                    ...
                    // Permission denied
                    var enableLocationLink = '<a href="javascript:void(0);" onclick="redirectToLocationSettings()">Click here</a>';
                    alert('Location access is denied. Please enable it in your device settings. ' + enableLocationLink);
                }
            });
        } else {
    ...
    function redirectToLocationSettings() {
        // This link will vary depending on the device's operating system
        window.location.href = 'intent://com.android.settings/com.android.settings.Settings$LocationSettingsActivity#Intent;scheme=android.intent.action.MAIN;end';
        window.location.href = 'app-settings:'; // You need to specify the correct link for each platform (iOS, Android, etc.)
    }
